...
#set mprfile "msc/test12/test12.mpr"
set sdltarget "C:/Battleships/Strategy/target/"
set sdlsystemname "Model"

set SIC_PATH $env(SIC_PATH)
regsub -all {\\} $SIC_PATH {/} SIC_PATH

if {$flagCLEAR} then {
	set files [glob -nocomplain -- *.obj *.exe *.lst test/*.* work/*.* ]
	foreach f $files {
		file delete $f
	}
...
if { $flagCOMPILERUN } then {
	...
	append srcfiles "C:/Battleships/Source/MapFunctions.c "
	append srcfiles "C:/Battleships/Source/ExternalFunction.c "
